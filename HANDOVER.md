...
**MINOR PRE-EXISTING QUIRK — SCN ref can be reused after a delete (found 23 Jun 2026):** the SCN ref generator uses `SCN-{year}-{COUNT(*)+1}`, so deleting an SCN lowers the count and a subsequent create can re-issue an already-used ref (e.g. `SCN-2026-0326` reappeared on a new SCN after an earlier test SCN with that ref was deleted). Low impact (refs aren't a unique key and deletes are admin-only), but a monotonic sequence (or max-suffix+1) would be more correct. Address opportunistically.
...
**Wireframe deviation (TC-approved, 24 Jun 2026):** Q3 child/off-PO items as real tracked stock. The wireframe has a basic "+ Add off-PO items" in Create-SCN Step 1 (WIREFRAME_INVENTORY.md:188) and a "Group by Item" in the Stock Register (:351), but does NOT cover: per-child ROS capture, showing the parent's inherited identity/WBS as read-only context, the block-on-unlinked-parent error, children appearing as receivable rows in Receipting, or children rendered NESTED under the parent (shared item_code) in the Stock Register. TC scoped these on top: child inherits commodity_id/equipment_tag/tag_number/wbs_code_snapshot from the parent at create (immutable snapshot), ros_date user-supplied; blocked if the parent is unlinked or has no WBS; child receipts create their own warehouse_stock row + grid location keyed on additional_item_id; Stock Register nests children under the parent via an is_child flag + shared item_code; subcontractor WBS bypass dropped. Approved by TC.
...
**Wireframe deviation (TC-approved, 25 Jun 2026):** Q4 containerised packaging + Q4.4 packaging UI + seal UI. The wireframe has NO container-type concept, NO seal-entry UI, and the SCN Detail Packages tab (WIREFRAME_INVENTORY.md:299) is a read-only table. TC scoped on top: ISO `container_types` reference table (9 types) + `scn_packages.container_type_id/container_no/seal_no`; a typed three-level hierarchy (typed container → sub-package → items, depth-3 cap); **seal governance** (`seal_no` set-once + reasoned re-seal + atomic failure-propagating `audit_log` write, container-only, single enforcement point `lib/sealGovernance.js`). UI (Q4.4, was deferred — built now as part of forwarder delegation): wizard Step 3 container-type picker on container rows; Logistics PackagesTab gains an Add/Edit form with container-type + nest-under selectors and **container_no (free) / seal_no (governed, with a re-seal-reason field that appears when changing an existing seal)**; container type + container/seal shown in the package table. Approved by TC.

**Wireframe deviation (TC-approved, 26 Jun 2026):** Pass 3a — SCN-side heat/cert→package linking (extends existing heat/lot tracking + Document Management; wireframe has neither). Additive only — does NOT touch receipt_lines/warehouse_stock/stock flows (that's 3b). Migration `migrate-heat-package-links.js`: `scn_heats += package_id` (FK→scn_packages ON DELETE SET NULL); `scn_documents += package_id + heat_id` (FKs ON DELETE SET NULL) + `document_type` enum += 'Mill Test Certificate'. Wizard Heats step gains an optional "In package" picker per heat (resolved server-side in the create txn via a capability-detected UPDATE pass — heats inserted before packages); SCN-detail Documents upload links a Mill Test Certificate to a package and/or heat (reuses the existing scn_documents upload); SCN-detail Packages tree shows per-package linked heats (🔥) + mill cert (📄 MTC). Existing receipting heat dropdown (scn_heats by scn_id) UNAFFECTED. Capability-detected for deploy-tolerance. Approved by TC.

**Wireframe deviation (TC-approved, 26 Jun 2026):** Pass 2 design-correctness fixes on the SCN wizard + packaging UI (TC walk-through findings, none in the wireframe). (1) Package-exceeds-container **dimension hard-block** when a sub-package exceeds its container's inner dims, with per-ISO-type relaxation derived from the container code suffix (DC/HC/RF enforce L/W/H; OT relaxes height; FR is out-of-gauge → not blocked) — shared helper `src/lib/packaging.ts`, applied on both surfaces. (2) **Multi-modal** transport: constituent-mode checkboxes + leg-notes field when mode=Multi-modal — **UI only; persistence FLAGGED/deferred** (mode enum has no 'multi' value and no column for the legs — pending TC schema decision; see HANDOVER_NEXT_SESSION). (3) **Packages-tree numbering**: top-level units (containers + loose) number consecutively 01,02,03 (display-only; the DB package_number is a flat sequence) and nested rows are labelled "Item". (4) **Forwarder** field on SCN-details step is now a **dropdown** of project forwarders (same source as the delegation picker), writing forwarder_name. (5) **Contents picker** shows only not-fully-packed lines with the **balance qty** ("X to pack"), reusing the existing allocation tally. Approved by TC.

**Wireframe deviation (TC-approved, 25 Jun 2026):** D5.1 container-first packaging UI. Revises the D5 packaging UI (which presented "container" as just-another-package-type behind a nest-under selector — TC found it undiscoverable on the live walk-through). New model on BOTH surfaces (wizard Step 3 + Logistics PackagesTab): **Add container** is a top-level action (pick ISO type → its inner dims show read-only; optional container_no/seal_no) → **Add package into this container** nests sub-packages → **Add loose package** for top-level leaves; items live in leaves only. Mirrors the Q4 backend guards (container top-level/no-items, depth-3) which are unchanged. Backend gap filled: the SCN-create txn (expediting `insertPkg`) now persists `container_no` (plain) + routes `seal_no` through the SAME `setSealNo` governance in-txn (so a wizard seal at creation is set-once/audited; the logistics POST already did). Verified live: container-first create (40HC + sub + loose), governed seal at creation (audit row `seal_initial` + hash), reasonless re-seal → 422, reasoned → 200, item-in-container → 422 backstop. Approved by TC (Option A).

**Wireframe deviation (TC-approved, 25 Jun 2026):** Forwarder-delegated packaging. The wireframe has NO concept of delegating packing to a forwarder. TC scoped: an expeditor at the wizard packaging step picks a packing scenario (internal / vendor / **forwarder**); choosing forwarder shows a forwarder picker and lets the SCN be created with packaging **unfinished** (relaxes the wizard's D2 `allFullyAllocated` gate for this path only). The delegated forwarder gets SCOPED write access (read/create/edit packages + set governed seals) to **that SCN only** via a router-level authorization carve-out keyed off the URL `:scnId` (never the body); a forwarder cannot reach packages on any SCN not delegated to them (cross-SCN leakage proven closed, backend + live). Hand-back: the forwarder marks packaging complete (`packaging_status` pending→complete, `packaging_completed_at`), visible to the expeditor. New schema on `shipment_control_notes`: `packed_by_type`, `packaging_delegated_to` (FK→users, the authz grant), `packaging_status`, `packaging_completed_at`. Forwarder-facing "📦 Packing (pending)" register filter (built on the existing `forwarder_user_id` scoping). Expeditor review: SCN-detail Overview shows "Packed by / Packaging" status; PackagesTab shows a delegation banner + Mark-complete action. The pre-existing `/status` `/dates` forwarder-write ownership gap is logged separately (NOT folded in) in HANDOVER_NEXT_SESSION.md. Approved by TC.
