...
CREATE TABLE `tender_approvals` (
  ...
  KEY `idx_ta_recommended_bid` (`recommended_bid_id`),
...
--
-- Table structure for table `tender_bid_lines`
--

DROP TABLE IF EXISTS `tender_bid_lines`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `tender_bid_lines` (
  ...
  `qty_proposed` decimal(15,3) NOT NULL,
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_tbl_bid_line` (`bid_id`,`tender_line_item_id`),
  KEY `idx_tbl_line` (`tender_line_item_id`),
...
CREATE TABLE `tender_line_items` (
  ...
  `qty_awarded` decimal(15,3) DEFAULT NULL COMMENT 'Quantity actually awarded on this line, written once by the award-to-PO handoff (generate-po). NULL = no award decision (still active, or released without an award); 0 = the award gave this line nothing; = qty_reserved for a full award; between 0 and qty_reserved for a partial award.',
  ...
  CONSTRAINT `chk_tli_awarded_coherence` CHECK ((((`status` = _utf8mb4'active') and (`qty_awarded` is null)) or ((`status` = _utf8mb4'converted') and (`qty_awarded` is not null) and (`qty_awarded` = `qty_reserved`)) or ((`status` = _utf8mb4'partial_released') and (`qty_awarded` is not null) and (`qty_awarded` > 0) and (`qty_awarded` < `qty_reserved`)) or ((`status` = _utf8mb4'released') and ((`qty_awarded` is null) or (`qty_awarded` = 0))))),
...
CREATE TABLE `tender_packages` (
  ...
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `criteria_locked_at` datetime DEFAULT NULL,
  `criteria_locked_by` int DEFAULT NULL,
  PRIMARY KEY (`id`),
  ...
  KEY `fk_tp_crit_locked_by` (`criteria_locked_by`),
  ...
  CONSTRAINT `fk_tp_crit_locked_by` FOREIGN KEY (`criteria_locked_by`) REFERENCES `users` (`id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=4241 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
