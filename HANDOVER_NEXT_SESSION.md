...
# Last commit: f31b4e2 (feat(audit): audit FMR PoC signature upload — completes item #22 document-action audit gap) — feat/three-design-features, PUSHED. All Pass-1/2/3 + walk-through fixes + forwarder doc/PoC carrier-auth + blob migration (LIVE IN PROD) + document Download/View pass + document-action audit gap (DEPLOYED + PROD-VERIFIED) DONE & pushed.
...
16. **▶ TODO (fresh session, NOT built) — Date-logic validation + "Create disabled" feedback ROOT-FIX (`CreateSCNWizard`).** Logged 28 Jun. **Problem:** the Create-SCN gate blocked silently on illogical dates (e.g. ETA before ETD, ETD/ETA before CCD) — correct to block, but NO reason surfaced. This is the **3rd instance of "correct gate, silent block"** (after untyped-container and dimension-violation, both already fixed by surfacing the reason in the Confirm banner). **Root fix wanted (NOT a one-off patch):** audit EVERY condition that can make `canCreate` false and ensure each surfaces a clear reason in the Confirm disable-reason banner — so no gate, present or future, can disable Create silently. The banner pattern already exists (allocation / untyped-container / dimension / forwarder reasons) — extend it to be exhaustive. **Resolve FIRST (read-only diagnosis):** does a date-logic validation actually EXIST, or did Create block for a DIFFERENT reason that coincidentally cleared when dates changed? Trace the real cause before building. **If no date rule exists, ADD** sensible date-ordering validation (logical CRD/CCD/ETD/ETA ordering) WITH its banner reason. Scope: which date fields, the correct ordering rules, hard-block vs warning.

17. **▶ TODO (fresh session, NOT built — FEATURE, needs scoping) — SCN status-lifecycle expansion (`logistics.js` status route + Update-Status UI).** Logged 28 Jun. **Problem:** the Update SCN Status dropdown is missing intermediate states — "Cargo picked up", "Cargo loaded onto vessel", "Cargo in transit" (observed: a forwarder login only had Customs Review / In Transit / Delivered etc., not the pickup/loading granularity). **Wanted:** add the missing intermediate statuses, settable by **freight_forwarder AND logistics AND expediting** roles (NOT forwarder-only). **This is a FEATURE, not a fix — scope BEFORE building:** (a) the full intended status lifecycle — what is the complete ORDERED set? (b) which roles may set which transitions; (c) any ordering/transition rules (the `NEXT_STATUSES` map in `logistics.js` governs valid transitions today); (d) the status enum migration (`shipment_control_notes.status` is an enum — adding states = an additive migration, supply-and-remove via QCO_admin). **Distinct from the carrier-doc auth (#15)** — this is the status-transition WORKFLOW, not document authorization.
...
20. **▶ TODO (fresh session, NOT built — missing FRONTEND affordance) — VDRL register: no file-upload control in the per-PO VDRL document view.** Logged 28 Jun. **Problem:** a VDRL line can be CREATED but there is **no control to upload the actual FILE** from the VDRL register / per-PO VDRL view. The **backend exists and is blob-wired**: `POST /:projectId/vdrl/documents/:docId/file` in `expediting.js` (memoryStorage → `blobStore.persist`; note it 503s until the `vdrl_documents` file-columns migration is applied — confirm that's done). **Scope:** decide where the upload control lives (per VDRL-document row in the register vs the doc detail), wire it to that route, and confirm the file routes through the blob-backed VDRL path (key `vdrl/<basename>`, served via the unified resolver).
...
21. **✅ DONE — Document Download/View on every doc-listing screen + foundational route-collision fix (`feat/three-design-features`, committed `a2f00ee`, pushed).** Consistent authed **Download + View** across all document-listing UIs via a new shared util `src/lib/fileAccess.ts` (`downloadFile` = authed blob → anchor; `viewFile` = authed blob → `URL.createObjectURL` → `window.open(blobURL)`) — **never** `window.open(apiURL)` (that omits the JWT → 401). Screens: Logistics Documents tab (was actions-less — now View+Download), Logistics PoC tab (+View), PO docs (+View), Document Inbox (real View; the old 👁 was a *mock* preview → relabelled ⓘ Details), Foundational certs (fixed + View), MTO revisions (+View/Download, graceful toast on file-less revisions). Traceability + FMR already used the correct pattern — left as-is.
    ...
    - **⏭ CARRIED FORWARD (out of scope here — NOT fixed): two `window.open(apiURL)` 401-class items** on *export/template* links (not document rows, same auth-header bug class — they omit the JWT so they 401 on a gated route): **WBS export** (`FoundWBSScreen.tsx` ~1630) and **one VDRL template link** (`ExpeditingScreen.tsx` ~346 — note a CORRECT axios-blob VDRL-template caller already exists at ~608, so :346 is an inconsistent duplicate). Convert both to the `fileAccess` util in a future pass. (Item #20's VDRL **upload**-button gap is separate and still open.)
