...
# Updated: 3 July 2026
# Last commit: aeaffaa (feat(vdrl): per-doc file upload + view/download in the VDRL doc-detail modal) — feat/three-design-features, PUSHED + deployed. This session added: #16 date-ordering feedback in Create-SCN wizard (e5585b5), COMMIT 1 window.open→401 pair — WBS export + VDRL template (b781e13), COMMIT 2 VDRL per-doc upload (aeaffaa); also the forwarder /status+/dates carrier-scoping fix (8b359eb). FIX 3 (MTO download) marked STALE — already ships. ⚠ Two live click-throughs outstanding (build-clean): #16 date feedback + #20 VDRL upload round-trip — verify at the walk-through. All Pass-1/2/3 + walk-through fixes + forwarder doc/PoC carrier-auth + blob migration (LIVE IN PROD) + document Download/View pass + document-action audit gap (DEPLOYED + PROD-VERIFIED) DONE & pushed.
...
1. **Azure credential pass — DELIBERATE FIRST TASK (both credential ops in ONE focused portal session).** Do these together, clear-headed, before anything else; the rest of the session then runs under the new credentials.
   ...
   - **1b. ✅ DONE / LIVE IN PROD — Blob Azure setup. BLOB MIGRATION COMPLETE.** Storage account **`qcommsuploads`** (**public blob access OFF**), private container **`uploads`**; **`AZURE_STORAGE_CONNECTION_STRING`** on **App Service config + local `server/.env`**; blob code **deployed to prod** (healthy boot, `@azure/storage-blob` installed via `SCM_DO_BUILD_DURING_DEPLOYMENT`). **Blob-hit path proven end-to-end LOCALLY** (write→blob with key `logistics/<basename>` not disk, serve→blob, dual-read serves new-blob alongside old-on-disk, forwarder-doc auth intact — carrier 201 / non-carrier 403) **AND confirmed IN PROD** (a real upload through the live app landed in blob — container count 0→1). Nothing left on blob. *(Aside: the Azure-CLI blob-count check needs an AD data-plane role on `qcommsuploads`; orthogonal — the app authenticates with the connection string, not AD.)* Map: `docs/BLOB_STORAGE_PLAN.local.md` (gitignored).
...
3. **Residual test-data cleanup in ZZ project 27 (admin DELETE — `qmat_app` can't, these tables are append-only for it).** Verify each before removing; keep canonical demo data.
   - **~12 throwaway test SCNs** (header rows in the ~2212–2235 range, many with no packages/lines) from this cycle's adversarial/live proofs.
   - **`po_documents` id 6** (po_id 1990, `is_current=0`) — from the blob procurement round-trip. Reused as a fixture in the Download/View pass (#21) and **restored to its prior state afterward** (no new residue); still needs admin DELETE.
   - **`traceability_certs` id 3617** (heat_ref `BLOBTEST-H1`, file_name `rt.txt`, disk file removed → download 404) — from the blob traceability round-trip.
   - **1 test doc on SCN 2233** (`scn_documents`: `Dashboard.pdf`, Commercial Invoice) uploaded through the live app during the **prod audit verification** (1 Jul). Delete the doc row + its blob if a clean demo is wanted. *(The corresponding `audit_log` id 545 stays — append-only tamper-evident trail, must NOT be deleted.)*
   - *(Both blob test rows are harmless — files gone, downloads 404 — but `qmat_app` lacks DELETE on those tables, so they need admin/QCO_admin to remove.)*
   - **1 test document on ZZ-SCN-0008** (PO `ZZ-PO-0008`, project 27): a `Commercial Invoice` named **`Dashboard.pdf`** uploaded during the **prod blob verification** (this is the file that proved prod writes to blob, count 0→1). Lives in `scn_documents` + the `qcommsuploads`/`uploads` blob container — delete the row (admin) AND the blob if a clean demo is wanted.
...
16. **✅ DONE this session (`feat/three-design-features`, committed `e5585b5`, pushed + deployed) — Date-ordering feedback in the Create-SCN wizard.** Logged 28 Jun; built 3 Jul. **DIAGNOSIS outcome (the read-only trace):** the original "Create blocked silently on illogical dates" was a **mis-read, not a silent gate** — `canCreate` had NO date condition, so the Create button always fired; the backend create route (`expediting.js:963`, `dateOrder([['CRD',crd],['CCD',ccd],['ETD',etd],['ETA',eta]])`) returned a descriptive **400**, which `handleCreate`'s catch surfaced as a **transient toast** — not the persistent Confirm banner like the other blockers. So the rule EXISTED (backend, authoritative) but its reason was surfaced inconsistently (post-submit toast vs pre-submit banner). Also confirmed the banner is **already exhaustive** for every real `canCreate` block (allocation / untyped-container / dimension / forwarder); the only non-banner condition is the transient `creating` flag. **FIX (frontend only — backend `dateOrder` untouched, stays authoritative):** new pure mirror `src/lib/dateOrder.ts` (comment: "mirror of server/utils/validate.js dateOrder — keep in sync; backend remains authoritative"; **7/7 byte-for-byte parity** proven vs the backend incl. the exact message strings). Wired into `CreateSCNWizard.tsx` at 3 surfaces: **inline** at the date fields on the SCN-details step (red message + red ETA border), the **Confirm banner** (a `• {message}` line alongside the other reasons), and the **`canCreate` gate** (`&& !dateOrderError` — Create now disables consistently with the other 4 hard-blocks). Rule: **CRD ≤ CCD ≤ ETD ≤ ETA** over whatever subset is present. tsc + build clean. **⚠ NOT yet live click-through-verified** (session token was expired; verify at the walk-through: enter ETA-before-CCD → inline + banner + Create disabled; fix → clears + enables). Backend-only-authoritative, no migration.

17. **▶ TODO (fresh session, NOT built — FEATURE, FULLY SCOPED, ready to build) — SCN status-lifecycle expansion (`logistics.js` status route + Update-Status UI).** Logged 28 Jun; **scoped 3 Jul (design locked below)**. **Problem:** the Update SCN Status dropdown is missing intermediate states — "Cargo picked up", "Cargo loaded", "Cargo in transit" (a forwarder login only had Customs Review / In Transit / Delivered, not the pickup/loading granularity). **LOCKED DESIGN:**
    - **(a) 8 canonical GENERIC status values** (mode-agnostic, stored):  `pending_pickup` → `picked_up` → `loaded` → `in_transit` → `arrived` → `customs` → `out_for_delivery` → `delivered`. One ordered spine for every transport mode.
    - **(b) Option A — generic status + mode-aware LABELS.** The stored value is the generic key; the UI renders a mode-specific label (e.g. `loaded` → "Loaded onto vessel" for sea, "Loaded onto aircraft" for air, "Loaded onto truck" for road). Label map is presentation-only; the DB never stores the pretty label.
    - **(c) NEW `destination_type` field (`site` / `warehouse` / `laydown` / `port`)** — an **orthogonal dimension, NOT a status fork.** It drives the `delivered` label ("Delivered to site" vs "…to warehouse" vs "…to laydown" vs "…to port") and gives reporting a clean cut. Do NOT branch the status spine on it — it's a separate column on `shipment_control_notes`.
    - **(d) Mode-filtered dropdown** — the Update-Status control shows the 8 states with mode-aware labels; destination_type is a small separate selector feeding the delivered label + reports.
    - **(e) FLEXIBLE transitions — NO `NEXT_STATUSES` map.** Any of the 8 may be set from any current state (real logistics skip/backtrack: e.g. courier skips customs; a correction reverts arrived→in_transit). Retire/skip the transition-guard map for SCN status; the ordered set is for display + reporting order only, not a hard state machine.
    - **(f) Roles — all 3 set status within EXISTING SCN scope: `freight_forwarder` AND `logistics` AND `expediting`.** ⚠ **Do NOT loosen the forwarder scoping** — the forwarder stays **carrier-scoped via `requireStatusDateAuth`** (`logistics.js`, keys off URL `:scnId` = `forwarder_user_id`; internal roles unchanged). This item ADDS states + a field, it does NOT touch who-can-write-which-SCN.
    - **(g) Audit + Timeline already wired** — the `status_update` action is already audited (`logistics.js` `writeAudit`), and status changes already flow to `scn_status_log` → the SCN Timeline. New states inherit both for free; no new audit/timeline code.
    - **(h) Migrations under QCO_admin (both additive + backward-compatible):** (i) extend the `shipment_control_notes.status` ENUM with any of the 8 keys not already present (supply-and-remove; existing rows keep their current value); (ii) add `destination_type` ENUM('site','warehouse','laydown','port') NULL. Both nullable/additive → deploy-tolerant; migration must travel with the code.
    - **Build order: BACKEND-FIRST** — migrations → status route accepts the new enum values + persists `destination_type` (capability-detect) → then the mode-aware label map + dropdown + destination selector in the UI → then adversarial role/scope proofs (forwarder cross-SCN still 403 via `requireStatusDateAuth`; all 3 roles set within scope). **Distinct from the carrier-doc auth (#15)** — this is the status-transition WORKFLOW, not document authorization.

18. **✅ DONE (CODE) — Azure Blob storage migration (`feat/three-design-features`, committed `d218232`, pushed).** Durability fix: file uploads move off ephemeral App Service local disk onto Azure Blob, via a **DUAL-READ FALLBACK** (blob-first, disk-fallback) so nothing breaks before/after cutover. `server/lib/blobStore.js` (`putFile`/`getFile`/`keyFor`/`persist`, degrade-safe — no connection string → returns null, falls back to disk, never throws). All **7 write paths** (logistics, procurement, vdrl, mto, foundational, traceability, fmr-poc) → `memoryStorage` → `blobStore.persist` (key stored on blob; byte-identical legacy disk shape on fallback). All **5 read points** (the `documents.js` unified resolver covering 6 modules + procurement/traceability/foundational/fmr-poc own routes) → blob-first then disk, still streamed through the authed API (no SAS/public URLs). Module prefixes aligned to the `documents.js` RESOLVERS keys (caught + fixed a write/read mismatch). **Proven:** keyFor normalizes all 3 stored shapes (relative/absolute/bare); graceful degradation with the string absent; live disk-fallback round-trips (logistics/procurement/traceability write→disk→serve, content intact); **logistics forwarder-doc auth NOT regressed** (auth is before the storage stage — 6/6 + HTTP 201/403). **✅ COMPLETE — LIVE IN PROD (item #1b done):** storage account `qcommsuploads` (public access off) + private `uploads` container, `AZURE_STORAGE_CONNECTION_STRING` on App Service + local `.env`, deployed to prod (`@azure/storage-blob` via `SCM_DO_BUILD_DURING_DEPLOYMENT`), **blob-hit path proven end-to-end locally** (write→blob/serve→blob/dual-read/auth) **AND confirmed in prod** (real upload landed in blob, count 0→1). Map: `docs/BLOB_STORAGE_PLAN.local.md` (gitignored).

19. **▶ TODO (fresh session, NOT built — missing FRONTEND affordance) — Document view/download button absent on the SCN Documents tab (and likely other doc views).** Logged 28 Jun. **Problem:** the Logistics SCN **Documents tab** (`LogisticsScreen.tsx`) lists each doc with a **DELETE (🗑) action but NO view/download control** — a user cannot open a document. The **backend download route already EXISTS and is blob-aware**: the dual-read authed resolver `GET /documents/:projectId/download/:docId` (`documents.js`) — same one the Document Inbox + PoC tab already use. **Scope:** add a per-row **view/download button** that calls that resolver (auth-carried blob fetch, mirror the existing `download()` in `DocumentsScreen.tsx` / PoC tab); audit the other doc views (PO docs, traceability, foundational certs) for the same gap. **★ Bonus — this is the first REAL UI exercise of the blob dual-read for an end user:** the SCN tab now holds BOTH a **new blob file** (e.g. the `Dashboard.pdf` prod test upload on ZZ-SCN-0008) AND **legacy on-disk files** (e.g. `PL-ZZ-0008.pdf`) — confirming the button serves **both** is excellent end-to-end dual-read validation in the live app.

20. **✅ DONE this session (`feat/three-design-features`, committed `aeaffaa`, pushed + deployed) — VDRL per-doc file upload + view/download.** Logged 28 Jun; built 3 Jul (COMMIT 2 of the file-access sweep). **Placement chosen:** the **VDRL doc-detail modal** (`ExpeditingScreen.tsx`, `selectedDoc`) — not a per-register-row action — so the control sits next to the doc's metadata. **UI:** a "Document file" section that shows either **📎 {file_name} + View + Download** (authed `viewFile`/`downloadFile` against the unified resolver `${API}/documents/${projectId}/download/vdrl:${docId}`; View uses fileAccess's inline-MIME fix) when a file is attached, OR an **↑ Upload file** control (hidden `<input type=file>`, "Uploading…" state) when none is. **Wired to the existing blob-backed route** `POST /:projectId/vdrl/documents/:docId/file` (multipart, field `file`) — backend UNTOUCHED (memoryStorage → `blobStore.persist`, key `vdrl/<basename>`; audits `vdrl_file_attached`; bumps `status`→`Under review` + stamps `submitted_date`; returns the updated row). On success the returned row is merged into both the docs list (spread-preserving joined fields) and the open modal, then a success toast. tsc + build clean. *(Confirmed the `vdrl_documents` file-columns migration is applied — the route no longer 503s.)*
    - **⚠ REQUIRED WALK-THROUGH CHECK (not yet done — session token was expired):** upload a real file to a **ZZ-27 VDRL doc** (throwaway, add to the ZZ-27 cleanup list) → confirm it **attaches**, the modal shows **View + Download**, the **status bumps to "Under review"** in BOTH the modal AND the VDRL register row, AND a **`vdrl_file_attached` audit row fires** (check `audit_log`). Build is clean; this is the live round-trip proof outstanding.

21. **✅ DONE — Document Download/View on every doc-listing screen + foundational route-collision fix (`feat/three-design-features`, committed `a2f00ee`, pushed).** Consistent authed **Download + View** across all document-listing UIs via a new shared util `src/lib/fileAccess.ts` (`downloadFile` = authed blob → anchor; `viewFile` = authed blob → `URL.createObjectURL` → `window.open(blobURL)`) — **never** `window.open(apiURL)` (that omits the JWT → 401). Screens: Logistics Documents tab (was actions-less — now View+Download), Logistics PoC tab (+View), PO docs (+View), Document Inbox (real View; the old 👁 was a *mock* preview → relabelled ⓘ Details), Foundational certs (fixed + View), MTO revisions (+View/Download, graceful toast on file-less revisions). Traceability + FMR already used the correct pattern — left as-is.
    - **★ Plus a real backend bug found+fixed via the live clicks — `fix(foundational)`:** the cert-download endpoint `GET /:projectId/certificates/:id/download` was **dead** — registered AFTER the generic `GET /:projectId/certificates/:entityType/:entityId`, which shadowed it (`/209/download` → `entityId='download'` → `Number→NaN` → SQL `Unknown column 'NaN'` → 500). `window.open` had masked it (blank tab). Fixed by moving the `/download` route **before** the generic route (Express matches in registration order). Cert list route still 200 (reorder safe).
    - **Proven:** blob **dual-read through the REAL buttons** — #1 Logistics four-way (blob+disk × Download+View all authed 200, content intact) + #5 foundational real clicks; #3 PO docs + #8 MTO verified via the exact button URLs (authed 200 + content from blob, no-auth 401, #8 file-less → graceful 404 toast). tsc+build clean; no console errors.
    - **⚠ DEPLOY: frontend AND backend** — the foundational route-order fix is a `server/routes/foundational.js` change, so the API must redeploy with the frontend.
    - **✅ DONE this session (`feat/three-design-features`, committed `b781e13`, pushed + deployed) — the two `window.open(apiURL)` 401-class items** (COMMIT 1 of the file-access sweep). Both *export/template* XLSX links omitted the JWT → 401 on their gated routes; both converted to `fileAccess.downloadFile` (authed axios blob → anchor; XLSX → download, not view): **WBS export** (`FoundWBSScreen.tsx` `exportSelected` ~1630 → `downloadFile(url, 'WBS_Export.xlsx')`, `fileAccess` import added) and the **VDRL template link** inside `VDRLUploadModal` (`ExpeditingScreen.tsx` ~347 → `downloadFile(url, 'QCO_VDRL_Template.xlsx')`, import added). Grep confirms no `window.open(${API}…)` remains at either site. tsc + build clean. (The correct axios-blob VDRL-template caller at ~608 is untouched — the duplicate at :347 is now consistent with it.)
    - **⚠ STALE — FIX 3 (MTO revision file download) needs NO work; it already ships.** During the file-access mapping this was flagged as a possible gap, but the MTO **Version History** table (`MTODetailScreen.tsx:576–611`) **already** has a File column with **View/Download** via `viewFile`/`downloadFile` against the `mto:<revId>` resolver (`fileAccess` imported at :29, graceful "No file stored" toast). No build was done and none is needed — this is covered by item #21's MTO-revisions work. Marked stale so it isn't re-picked-up.

22. **✅ DONE + DEPLOYED + VERIFIED IN PROD — Document-action audit gap: uploads/deletes now audited in all 3 modules.** Built module-by-module, chain-verified after each. **Commits `cb3f859` (logistics doc upload+delete), `444b01b` (foundational cert upload+delete), `f31b4e2` (FMR PoC signature upload) — all pushed + deployed.** 5 routes now write to the tamper-evident `audit_log`: `document_uploaded`/`document_deleted` (logistics, via `writeAudit`), `certificate_uploaded`/`certificate_deleted` (foundational, via `audit`), `poc_signature_uploaded` (FMR, via `writeAudit`) — each non-blocking (matches the module's existing swallow-error helper). After each module: exercised the route, `verifyChain` → `verified`, `POST /checkpoint` → re-`verified` (head/count match). **PROD-CONFIRMED (read-only, 1 Jul):** a real upload through the deployed app wrote **`audit_log` id 545** (`document_uploaded`, `scn:2233`, proj 27, Dashboard.pdf, `row_hash` populated by the prod trigger), and `verifyChain` against prod returned **`verified` with 545 included** (contentBroken null → prod trigger hashes to the canonical `rowHashExpr`). The forwarder-tied logistics gap (`d1133f4`) is closed. **⚠ DEPLOY:** backend-only, no migration (travels with the API). *Original diagnosis retained below for the record.*
    - **DIAGNOSIS (retained):** `audit_log` is the **hash-chained, tamper-evident, append-only** trail (`lib/auditChain.js`: `rowHashExpr`/`foldChain`/`sealCheckpoint`/`verifyChain`); the Audit-Trail tab (`routes/audit.js`) only READS + verifies. Each module has its own write helper. **The 5 routes that were NOT audited (now fixed):**
    - **UPLOADS unaudited (3):** logistics `POST /scn/:scnId/documents` (SCN docs **+ PoC + mill certs**) → use **`logistics.writeAudit`**; foundational `POST /:projectId/certificates/:entityType/:entityId` → **`foundational.audit`**; materialcontrol `POST /:projectId/fmr/pickup/:pickupId/signature` (FMR PoC) → **`materialcontrol.writeAudit`**.
    - **DELETES unaudited (2):** logistics `DELETE /scn/:scnId/documents/:docId`; foundational `DELETE /:projectId/certificates/:id`.
    - **Already audited — DO NOT touch:** procurement `signed_po_uploaded`, vdrl `vdrl_file_attached`, mto `UPLOAD_REVISION`, traceability `cert_received`/`cert_uploaded`. **Downloads ARE audited** via `documents.auditDownload` (`document_downloaded`).
    - **CONTEXT — inversion + forwarder tie-in:** downloads are audited but uploads/deletes in these 3 modules aren't — the trail records who **READ** a doc, not who **added/removed** it. ⚠ **Ties to `d1133f4`:** forwarders are now authorized to upload/delete **logistics** docs (the unaudited routes) → a forwarder upload/delete leaves **NO audit trace**. The **forwarder-tied logistics gap is the priority**.
    - **FIX SHAPE (fresh head, careful — writes into the hash-chain):** add the module's existing per-helper audit call to those 5 routes, matching each module's existing action-naming convention. Decide per route: **non-blocking** (match the module's existing swallow-error pattern — these helpers `catch→console.error`) vs **in-txn-rollback** (the `lib/sealGovernance.js` failure-propagating pattern) — for a pure doc upload/delete, non-blocking is consistent with the existing helpers, but confirm with TC. **HARD REQUIREMENT:** after adding entries, run **`verifyChain`** / the Audit-tab chain verification and confirm the chain **STILL SEALS/verifies** — new entries must not break tamper-evidence. Build on a fresh head; verify the chain after.

23. **▶ TODO (fresh session, NOT built — scope only) — Audit-viewer UX coherence: surface the full tamper-evident `audit_log` in a user-facing view.** Logged 1 Jul. **Problem:** the comprehensive, hash-chained `audit_log` (document upload/delete — now incl. the item-#22 additions — plus security events, status changes, downloads, CRUD across every module) has **no coherent user-facing viewer**. The PO **"Audit Trail" tab is scoped only to milestone/note changes** — it does NOT surface `audit_log`, so it can't show (or verify) the tamper-evident trail. For a compliance-oriented materials system, this is a real gap. **Scope:** a proper **audit viewer over the whole `audit_log`** — filterable by entity_type / action / user / project / date, with a per-row before/after view, and surfacing the **chain-verify status** (`GET /api/audit/verify`) + a seal action (`POST /api/audit/checkpoint`, gated to admin/auditor). The read-only backend already exists (`routes/audit.js` — list + `/verify` + `/checkpoint`); this is primarily a **frontend** build (a dedicated Audit screen), possibly wired into the existing Audit nav item. **Distinct from** the Timeline (status/dates — a different table + purpose) and from the PO milestone/note "Audit Trail" tab. Confirm the RBAC (audit `can_view`) gates who sees it.
