...
# Last commit: d218232 (feat(storage): Azure Blob dual-read migration) — feat/three-design-features, PUSHED. All Pass-1/2/3 + walk-through fixes + forwarder doc/PoC carrier-auth + blob migration DONE & pushed. Blob Azure setup now LIVE (qcommsuploads, deployed to prod); only a live-prod upload check remains (walk-through).
...
1. **Azure credential pass — DELIBERATE FIRST TASK (both credential ops in ONE focused portal session).** Do these together, clear-headed, before anything else; the rest of the session then runs under the new credentials.
   ...
   - **1b. ✅ DONE / LIVE — Blob Azure setup.** Storage account **`qcommsuploads`** created (**public blob access OFF**), private container **`uploads`**; **`AZURE_STORAGE_CONNECTION_STRING`** set on **App Service config + local `server/.env`**; blob code **deployed to prod** (healthy boot, `@azure/storage-blob` installed via `SCM_DO_BUILD_DURING_DEPLOYMENT`). **Blob-hit path proven END-TO-END locally against the real account** (write→blob with key `logistics/<basename>` not disk, serve→blob, dual-read serves new-blob alongside old-on-disk, forwarder-doc auth intact — carrier 201 / non-carrier 403). **REMAINING (do in the walk-through, step 2):** one real upload **through the live PROD app** to confirm prod writes to blob. *(Note: an Azure-CLI blob-count check is blocked by an AD data-plane role on `qcommsuploads` — orthogonal: the app authenticates with the connection string, not AD, and the local end-to-end proof already exercised that exact credential. Just eyeball one prod upload.)* Map: `docs/BLOB_STORAGE_PLAN.local.md` (gitignored).
