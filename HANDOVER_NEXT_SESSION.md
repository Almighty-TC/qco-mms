...
14. **⚠ FLAGGED BACKEND NEED — Multi-modal transport storage (Pass 2, Item 2).** The SCN wizard now has a Multi-modal UI (constituent-mode checkboxes Sea/Air/Road/Rail/Courier + a leg-notes field), but **persistence is NOT wired** because the schema can't hold it: `shipment_control_notes.mode` is `enum('air','sea','road','rail','courier')` — **no 'multi' value** — and there's no column for the constituent legs or leg detail. **Decision needed before wiring:** (a) add `'multi'` to the `mode` enum + a `transport_modes VARCHAR/JSON` column (e.g. `'sea,road'`) + a leg-notes column (or reuse `notes`); or (b) store legs in the existing `notes` text. Recommend (a) — a small additive migration. Until decided, the multi-modal UI captures the selection in wizard state but the create payload drops it (and a bare `mode='multi'` would fail the enum). Build the migration + wire create/display once TC picks the shape.
