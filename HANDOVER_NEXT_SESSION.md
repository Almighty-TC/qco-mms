...
# Last commit: a2f00ee (feat(ui): authed Download+View on all doc screens + fix(foundational): dead cert-download route collision) — feat/three-design-features, PUSHED. All Pass-1/2/3 + walk-through fixes + forwarder doc/PoC carrier-auth + blob migration (LIVE IN PROD) + document Download/View pass DONE & pushed.
...
22. **▶ TODO (fresh session, NOT built — DIAGNOSED this session; integrity-sensitive write path) — Document-action audit gap: uploads/deletes unaudited in 3 modules.** Logged 28 Jun. **Read-only diagnosis done.** `audit_log` is the **hash-chained, tamper-evident, append-only** trail (`lib/auditChain.js`: `rowHashExpr`/`foldChain`/`sealCheckpoint`/`verifyChain`); the Audit-Trail tab (`routes/audit.js`) only READS + verifies. Each module has its own write helper. **Most document actions ARE audited — but these are NOT:**
