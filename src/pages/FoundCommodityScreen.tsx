...
// Resizable column defaults — commodity library (9 cols).
const FC_W   = [120, 200, 70, 110, 110, 100, 150, 100, 90]
...
export const FoundCommodityScreen = ({ dark, projectId, projectName, onBack }: {
  ...
  const rt = useResizableTable('found_commodity', FC_W, FC_MIN)
  ...
  const renderRows = (list: Commodity[]) => list.map(c => (
    <tr key={c.id} style={{ background: dark ? '#1e293b' : '#fff', borderBottom: bd }}
      ...
      <td data-align="center" data-col="status" style={{ padding: '9px 10px' }}><StatusPill status={c.status} /></td>
  ...
  const tableHead = (
    <thead>
      <tr style={{ background: dark ? '#0f172a' : '#f4f7fb', position: 'sticky', top: 0, zIndex: 2 }}>
        {[['code','Code'],['name','Name'],['uom','UOM'],['wbs_code','WBS'],['trace_level','Trace level'],['preservation','Preserve'],['preferred_vendor','Vendor'],['status','Status']].map(([k,l], i) => (
          <th key={k} style={{ ...rt.thStyle(i), ...thStyle(k) }} onClick={() => toggleSort(k)}>{l}{sortArrow(k)}{rt.handle(i, dark)}</th>
        ))}
        <th style={{ ...rt.thStyle(8), padding: '8px 10px', borderBottom: bd, textAlign: 'center', fontSize: 10, fontWeight: 700, color: '#64748b', letterSpacing: '0.08em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>Actions</th>
      </tr>
    </thead>
  )
